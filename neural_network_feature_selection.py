...
def neural_network(XX,yy,shape,feature_name):
    XX = pd.DataFrame(XX)
    yy = pd.DataFrame(yy)
    ...
    accuracy_train, accuracy_test, precision, recall, predict, predict_len, training_time, testing_time = neural_network_predict(x_train, x_test, y_train, y_test, shape,feature_name)
...
#   print('Feature selection : {}'.format(feature['feature']))
  neural_network(feature['XX'],yy,len(feature['XX'][0]),feature['feature'])
